...
const EditPrebuiltContents = () => {
  ...
  const getPrebuiltContent = async () => {
    try {
      const headers = {
        Authorization: localStorage.getItem("accessToken"),
      };
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/api/v1/admin/products/${productId}/prebuilt-contents/${id}/edit`,
        { headers }
      );

      console.log(response.data.data);

      const prebuilt = response.data.data;
      console.log(prebuilt);
      setName(prebuilt.name);
      setSelectedIconPrefix(prebuilt.icon.split(" ")[0]);
      setSelectedIconValue(prebuilt.icon.split(" ")[1]);
      setSelectedIconShow({ label: prebuilt.icon, value: prebuilt.icon });
      setTextColor(prebuilt.textColor);
      setDescription(prebuilt.description);
      setDestination(prebuilt.destination);
      setTextContent(prebuilt.textContent);
      setMyClass(prebuilt.class);
    } catch (error) {
      console.log(error);
    }
  };

  const updatePrebuiltContent = async () => {
    try {
      const headers = {
        Authorization: localStorage.getItem("accessToken"),
      };
      ...
      <div className="page-content">
        <Container fluid>
          <div className="d-flex justify-content-between mb-2">
            <h4>{t("Edit Pre-built Contents")}</h4>
          </div>
          <Row>
            <Col md={12}>
              <Card>
          ...
          <Row>
            <Col md={12}>
              <div className="d-grid gap-2">
                <Button
                  onClick={updatePrebuiltContent}
                  type="button"
                  className="btn btn-warning"
                >
                  Update Pre-built Contents
                </Button>
              </div>
            </Col>
          </Row>
...
export default EditPrebuiltContents;
